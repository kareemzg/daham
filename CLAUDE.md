...
- A build ships one season, not the year. `Mansions.SHIPPED` is seven, so the export carries spring (`m01`–`m07`, 140 levels) and excludes the rest, along with `scenes/dev/*`, `tools/*` and `docs/*`. The dictionary is a pipeline input, never read at runtime, and shipping it would add three quarters of a megabyte nothing opens. After touching the filter, export a pack and list what is in it; a glob nobody has seen the output of is a guess.
